            ...
            <div className="mt-6 flex gap-3">
              {[Instagram, Youtube, Twitter, Facebook].map((Icon, i) => (
                <a
                  key={i}
                  href="#"
                  className="grid h-10 w-10 place-items-center rounded-full border border-white/10 text-white/60 transition hover:border-brand-red hover:text-brand-red"
                >
                  <Icon size={18} />
                </a>
              ))}
            ...
            <div key={col.title}>
              ...
              <ul className="space-y-2.5">
                {col.links.map(([label, href]) => (
                  <li key={label}>
                    <Link href={href} className="text-sm text-white/55 transition hover:text-cream">
                      {label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="mt-14 flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-8 text-xs text-white/45 sm:flex-row">
          ...
          <div className="flex items-center gap-2">
            {["UPI", "VISA", "Mastercard", "RuPay", "COD"].map((m) => (
              <span
                key={m}
                className="rounded-md border border-white/10 bg-white/10 px-2 py-1 text-[10px] font-bold tracking-wide text-white/70"
              >
                {m}
              </span>
            ))}
