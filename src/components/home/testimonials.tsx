...
function Row({ reverse }: { reverse?: boolean }) {
  ...
  return (
    <div className="flex w-max animate-marquee gap-5" style={reverse ? { animationDirection: "reverse" } : undefined}>
      {[...list, ...list].map((r, i) => (
        <figure key={i} className="w-[360px] shrink-0 rounded-2xl border border-white/10 bg-ink-800 p-6">
          <div className="flex items-center justify-between">
            <Stars rating={r.rating} size={14} />
            <Quote className="text-white/15" size={26} />
          </div>
          <blockquote className="mt-4 text-sm leading-relaxed text-white/75">“{r.text}”</blockquote>
          <figcaption className="mt-5 flex items-center gap-3">
            <span className="relative grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-xl border border-white/10 bg-white">
            ...
            <div>
              ...
              <p className="text-xs text-white/45">{gimgs[i % gimgs.length].name}</p>
            </div>
          </figcaption>
        </figure>
      ))}
...
export function Testimonials() {
  return (
    <section className="relative overflow-hidden border-y border-white/10 bg-ink-800/40 py-24">
