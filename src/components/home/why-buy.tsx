      ...
      <div className="container-wide">
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-3">
          {reasons.map((r, i) => (
            <Reveal key={r.title} i={i}>
              <motion.div
                whileHover={{ y: -6 }}
                className="group h-full rounded-3xl border border-black/10 bg-white p-8 shadow-sm transition-shadow hover:shadow-card"
              >
        ...
        <Reveal>
          <div className="mt-10 grid gap-px overflow-hidden rounded-3xl border border-black/10 bg-black/[0.06] sm:grid-cols-2 lg:grid-cols-4">
            {promises.map((p) => (
              <div key={p.label} className="flex items-center gap-3 bg-white px-6 py-5">
