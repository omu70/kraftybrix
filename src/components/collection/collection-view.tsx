...
export function CollectionView({ products }: { products: Product[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return r;
  }, [products, cats, maxPrice, diffs, inStockOnly, minRating, sort]);

  // Heading reflects a single selected category, else the full collection.
  const activeCat = cats.length === 1 ? categories.find((c) => c.name === cats[0]) : undefined;
  const heading = activeCat ? activeCat.name : "Every dream, in brick";
  const subheading = activeCat ? activeCat.blurb : "The full collection.";
