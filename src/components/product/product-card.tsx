...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <motion.div
      ...
      className="group relative rounded-2xl border border-white/10 bg-ink-800 p-3 transition-shadow duration-300 hover:border-white/20 hover:shadow-card"
    >
      {/* badges */}
      ...
      <button
        ...
        className={cn(
          "absolute right-5 top-5 z-10 grid h-9 w-9 place-items-center rounded-full border border-white/10 bg-ink-900/70 backdrop-blur transition",
          wished ? "text-brand-red" : "text-white/60 hover:text-cream"
        )}
      >
      ...
      <Link href={`/product/${product.slug}`}>
        ...
        >
          ...
          <div className="absolute inset-x-3 bottom-3 flex translate-y-3 gap-2 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
            <span className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-white/10 py-2 text-xs font-medium backdrop-blur">
      ...
      <div className="px-2 pb-1 pt-4">
        ...
        <p className="mt-0.5 text-sm text-white/50">{product.pieces.toLocaleString("en-IN")} pieces</p>

        <div className="mt-3 flex items-center justify-between">
          <div className="flex items-baseline gap-2">
            <span className="font-display text-xl font-bold">{formatPrice(effPrice)}</span>
            {off > 0 && <span className="text-sm text-white/40 line-through">{formatPrice(product.price)}</span>}
          </div>
          <button
            onClick={() => {
              add(product);
            ...
            className="grid h-10 w-10 place-items-center rounded-full bg-brand-gold text-ink-900 transition hover:brightness-110 disabled:bg-white/10 disabled:text-white/40"
          >
            <ShoppingBag size={17} />
          </button>
        </div>
