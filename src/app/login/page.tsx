...
export default function LoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res?.error) {
        setErr(mode === "login" ? "Wrong email or password." : "Account created — please sign in.");
      ...
      <div className="w-full max-w-md">
        <div className="rounded-3xl border border-black/10 bg-ink-800 p-8">
          ...
          <form className="space-y-3" onSubmit={onSubmit}>
            {mode === "register" && (
              <input value={name} onChange={(e) => setName(e.target.value)} required placeholder="Full name" className="w-full rounded-xl border border-black/15 bg-ink-900 px-4 py-3 text-sm outline-none focus:border-brand-red" />
            )}
            <div className="relative">
              <Mail size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-black/40" />
              <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" required placeholder="Email" className="w-full rounded-xl border border-black/15 bg-ink-900 py-3 pl-10 pr-4 text-sm outline-none focus:border-brand-red" />
            </div>
            <div className="relative">
              <Lock size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-black/40" />
              <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" required placeholder="Password" className="w-full rounded-xl border border-black/15 bg-ink-900 py-3 pl-10 pr-4 text-sm outline-none focus:border-brand-red" />
            </div>

            {err && <p className="text-sm text-brand-red">{err}</p>}

            <Button type="submit" size="lg" disabled={busy} className="w-full group">
          ...
          <p className="mt-5 text-center text-sm text-black/50">
            ...
            <button onClick={() => { setMode(mode === "login" ? "register" : "login"); setErr(""); }} className="font-semibold text-brand-red hover:underline">
